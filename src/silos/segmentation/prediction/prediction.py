...
def dice_coef(y_true, y_pred, threshold=0.8):
    ...
    return (2. * intersection + 0.00001) / (tf.reduce_sum(y_true_f) + tf.reduce_sum(y_pred_f) + 0.00001)
...
class SilosSegmentation:
    ...
    def __init__(self):
        with importlib.resources.path('silos.segmentation.prediction.models', 'segment1.h5') as model_path:
    ...
    def predict(self, to_predict: str, from_buffer: bool = False) -> Any:
        ...
        if from_buffer:
            ...
            img = load_img(
                to_predict, target_size=(32, 32, 1), grayscale=False)
