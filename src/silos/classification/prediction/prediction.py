...
class SilosClassification:
    ...
    def __init__(self) -> None:
        with importlib.resources.path('silos.classification.prediction.models', 'cnn_nath.hdf5') as model_path:
    ...
    def predict(self, to_predict: str, from_buffer: bool = False) -> Any:
        ...
        if from_buffer:
            ...
            results = []
            for f in [file for file in os.listdir(to_predict) if os.path.isfile(os.path.join(to_predict, file))]:
                img = load_img(os.path.join(to_predict, f), target_size=(32, 32, 1), grayscale=False)
                img = img_to_array(img)
                img = img/255
                img = img[None, :]
                prediction = self.model.predict(img)
