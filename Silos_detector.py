...
img_file_buffer = st.file_uploader("Upload your satellite pictures", accept_multiple_files=True,
...
if img_file_buffer is not None:
    ...
    for uploaded_file_buffer in img_file_buffer:
        ...
        with tab:
            ...
            st.write("Foodix Classification model returns the probability of a silo being present on the uploaded "
            ...
            st.markdown(f"<h1 style='text-align: center;'>" + str(prediction) + "</h1>", unsafe_allow_html=True)

            st.markdown(f"<h1 style='text-align: center;'>Segmentation</h1>", unsafe_allow_html=True)
            st.write("Foodix Segmentation model tries to locate potential silos on the uploaded satellite picture.")
            ...
            predimask = Image.fromarray(np.uint8(predimask.get_cmap()(predimask.get_array()) * 255))
